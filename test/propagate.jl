...
@testset "Model 1" begin
    ...
    mcres = NeXLUncertainties.mcpropagate(
        TestMeasurementModel(),
        inputs,
        100000,
...
@testset "K-Ratio test" begin
    ...
    ICharModel(id::String) =
        IChar(id) ∘
        (NormI("low", id) | NormI("peak", id) | NormI("high", id) | norm2char(id))
    ...
    ICharModelP(id::String) =
        IChar(id) ∘ (NormI("low", id)^NormI("peak", id)^NormI("high", id)^norm2char(id))
    TotalModelP(id::String) =
        (KRatioModel(id)^retain) ∘ (ICharModelP("std")^ICharModelP("unk") | retain)
